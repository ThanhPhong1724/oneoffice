...
<form method="post" action="options.php" class="cnb-container">
	...
	<table class="form-table <?php echo $active_tab == 'basic_options' ? 'nav-tab-active' : ''; ?>">
		...
		<tr valign="top" class="button-text">
			...
			<td>
				<input id="buttonTextField" type="text" name="cnb[zalo]" value="<?php echo isset($cnb_options['zalo']) ? $cnb_options['zalo'] : ''; ?>" />
			</td>
		</tr>
		...
		<tr valign="top" class="button-text">
			...
			<td>
				<input id="messengerField" type="text" name="cnb[messenger]" value="<?php echo isset($cnb_options['messenger']) ? $cnb_options['messenger'] : ''; ?>" />
				<p class="description">Username/ID hoặc đường dẫn Facebook Messenger (Ví dụ: <code>oneoffice.vn</code> hoặc <code>https://m.me/oneoffice.vn</code>)</p>
			</td>
		</tr>
	</table>
...
if(get_option('cnb') && !is_admin()) {
	...
	if($cnb_enabled) {
		...
		function cnb_footer() {
			...
			// Extract and build Zalo and Messenger URLs dynamically
			$messenger_id = isset($cnb_options['messenger']) ? trim($cnb_options['messenger']) : '';
			if (empty($messenger_id)) {
				$messenger_id = 'oneoffice.vn';
			}
			if (strpos($messenger_id, 'http') === 0) {
				$messenger_url = $messenger_id;
			} else {
				$messenger_url = 'https://m.me/' . ltrim($messenger_id, '/');
			}

			$zalo_id = isset($cnb_options['zalo']) ? trim($cnb_options['zalo']) : '';
			if (empty($zalo_id)) {
				$zalo_id = '0966681616';
			}
			if (strpos($zalo_id, 'http') === 0) {
				$zalo_url = $zalo_id;
			} else {
				$zalo_url = 'https://zalo.me/' . ltrim($zalo_id, '/');
			}
			...
			if (!function_exists('cnb_hex2rgb')) {
				function cnb_hex2rgb($hex) {
					$hex = str_replace("#", "", $hex);
					if(strlen($hex) == 3) {
			...
			$theme_color = ($cnb_options['color'] == '#00bb00' || empty($cnb_options['color'])) ? '#00a1b2' : $cnb_options['color'];
			$theme_color_rgb = cnb_hex2rgb($theme_color);

			$cnb_call_link = '<style>
:root {
    --cnb-theme-color: '.esc_attr($theme_color).';
    --cnb-theme-color-rgb: '.esc_attr($theme_color_rgb).';
}
</style>';
			$cnb_call_link .= '<div id="metu">
				<div class="oo-mobile-bar">
					<a href="tel:'.$cnb_options['number'].'" class="oo-mobile-bar__item oo-mobile-bar__item--phone" '.$cnb_onclick_events.'>
						<svg viewBox="0 0 24.04 25.08" class="oo-mobile-bar__icon"><path d="M7.45,4.29c0,0,1.52,1.53,0.27,3.19s-2.15,2.93,1.2,6.85s6.01,4.19,7.25,3.37c1.24-0.81,2.27-1.43,3.36-0.73 c1.09,0.7,5.07,2.8,4.45,4.56c-0.62,1.76-2.75,4.4-7.79,3.29S-1.56,15.91,0.16,3.62c0,0,0.48-2.48,3.33-3.58 c0,0,0.54-0.25,1.2,0.46S7.45,4.29,7.45,4.29z"/></svg>
						<span class="oo-mobile-bar__label">Gọi ngay '.$cnb_options['text'].'</span>
					</a>
					<a href="'.$zalo_url.'" target="_blank" class="oo-mobile-bar__item oo-mobile-bar__item--zalo">
						<span class="oo-mobile-bar__label">Chat Zalo</span>
					</a>
					<a href="'.$messenger_url.'" target="_blank" rel="noopener" class="oo-mobile-bar__item oo-mobile-bar__item--messenger">
						<svg viewBox="0 0 36 36" class="oo-mobile-bar__icon"><path fill="#fff" d="M18 2C9.163 2 2 8.728 2 17c0 4.566 1.992 8.66 5.2 11.558V34l5.052-2.773C13.603 31.7 15.763 32 18 32c8.837 0 16-6.728 16-15S26.837 2 18 2Zm1.597 20.218-4.072-4.34-7.953 4.34 8.745-9.283 4.176 4.34 7.845-4.34-8.741 9.283Z"/></svg>
						<span class="oo-mobile-bar__label">Messenger</span>
					</a>
				</div>
			</div>';
			$cnb_call_link .= '<div class="oo-desktop-cta">
				<a href="tel:'.$cnb_options['number'].'" class="oo-desktop-cta__item oo-desktop-cta__item--phone" '.$cnb_onclick_events.'>
					<svg viewBox="0 0 24.04 25.08" class="oo-desktop-cta__icon"><path d="M7.45,4.29c0,0,1.52,1.53,0.27,3.19s-2.15,2.93,1.2,6.85s6.01,4.19,7.25,3.37c1.24-0.81,2.27-1.43,3.36-0.73 c1.09,0.7,5.07,2.8,4.45,4.56c-0.62,1.76-2.75,4.4-7.79,3.29S-1.56,15.91,0.16,3.62c0,0,0.48-2.48,3.33-3.58 c0,0,0.54-0.25,1.2,0.46S7.45,4.29,7.45,4.29z"/></svg>
					<span class="oo-desktop-cta__tooltip">Gọi ngay: '.$cnb_options['text'].'</span>
				</a>
				<a href="'.$zalo_url.'" target="_blank" class="oo-desktop-cta__item oo-desktop-cta__item--zalo">
					<svg viewBox="0 0 27.64 25.07" class="oo-desktop-cta__icon"><path d="M12.92,12.02c-0.46-0.26-1.03-0.14-1.38,0.26c-0.48,0.55-0.48,1.65-0.02,2.22c0.34,0.42,0.93,0.56,1.38,0.3 c0.53-0.3,0.71-0.8,0.72-1.39C13.61,12.84,13.46,12.33,12.92,12.02z M12.92,12.02c-0.46-0.26-1.03-0.14-1.38,0.26 c-0.48,0.55-0.48,1.65-0.02,2.22c0.34,0.42,0.93,0.56,1.38,0.3c0.53-0.3,0.71-0.8,0.72-1.39C13.61,12.84,13.46,12.33,12.92,12.02z  M13.81,0C6.19,0,0,5.61,0,12.53c0,3.23,1.37,6.16,3.58,8.39H3.56c0,0,0.03,0.03,0.08,0.06c0.07,0.07,0.13,0.13,0.2,0.19 c0.21,0.26,0.4,0.72,0.02,1.38c-0.5,0.86-1.49,1.67-2.23,2.06c-0.15,0.08-0.13,0.31,0.03,0.34c1.18,0.23,2.56,0.13,3.83-0.24 c1.47-0.43,2.21-0.9,3.74-0.36v-0.01c1.44,0.46,2.98,0.73,4.59,0.73c7.64,0,13.82-5.61,13.82-12.53S21.45,0,13.81,0z M8.55,16.42 c-1.44,0.02-2.88,0.01-4.32,0.01c-0.43,0-0.84-0.08-1.07-0.5c-0.24-0.46-0.08-0.88,0.2-1.27c1.05-1.41,2.11-2.82,3.16-4.23 c0.1-0.13,0.25-0.23,0.26-0.5h-2.3c-0.21,0-0.42,0.02-0.62-0.03C3.43,9.84,3.18,9.57,3.18,9.13c0-0.46,0.25-0.79,0.71-0.79 c1.53-0.03,3.07-0.04,4.6,0c0.66,0.02,0.95,0.62,0.68,1.24C8.99,10,8.71,10.36,8.45,10.72c-0.98,1.3-1.94,2.59-3,4 c1.08,0,2.03-0.01,2.98,0c0.45,0.01,0.88,0.08,0.99,0.63C9.54,16.03,9.24,16.42,8.55,16.42z M15.41,15.37 c0,0.44-0.07,0.84-0.51,1.05c-0.43,0.21-0.81,0.1-1.11-0.27c-0.08-0.1-0.14-0.13-0.28-0.05c-1.8,0.96-3.38,0.35-3.95-1.53 c-0.38-1.23-0.22-2.38,0.58-3.43c0.79-1.03,2.45-1.39,3.51-0.35c0.3-0.36,0.63-0.67,1.16-0.46c0.53,0.23,0.59,0.68,0.59,1.18 C15.41,12.8,15.41,14.09,15.41,15.37z M17.94,15.51c-0.01,0.64-0.33,1.02-0.88,1.01c-0.54,0-0.87-0.38-0.87-1.03 c-0.01-2.28-0.01-4.54,0-6.81c0-0.68,0.31-1.03,0.86-1.04c0.57-0.02,0.88,0.36,0.89,1.05V15.51z M23.04,16.26 c-1,0.55-2.04,0.53-3.03-0.07c-1.45-0.87-1.9-3.09-0.96-4.63c0.85-1.38,2.68-1.83,4.07-0.98c1.05,0.65,1.45,1.65,1.47,2.85 C24.56,14.64,24.14,15.66,23.04,16.26z M22.2,12.07c-0.48-0.33-1.1-0.23-1.48,0.23c-0.48,0.57-0.49,1.63-0.01,2.21 c0.37,0.43,1.03,0.55,1.48,0.23c0.46-0.32,0.61-0.8,0.62-1.34C22.8,12.87,22.66,12.4,22.2,12.07z"/></svg>
					<span class="oo-desktop-cta__tooltip">Chat Zalo</span>
				</a>
				<a href="'.$messenger_url.'" target="_blank" rel="noopener" class="oo-desktop-cta__item oo-desktop-cta__item--messenger">
					<svg viewBox="0 0 36 36" class="oo-desktop-cta__icon"><path fill="#fff" d="M18 2C9.163 2 2 8.728 2 17c0 4.566 1.992 8.66 5.2 11.558V34l5.052-2.773C13.603 31.7 15.763 32 18 32c8.837 0 16-6.728 16-15S26.837 2 18 2Zm1.597 20.218-4.072-4.34-7.953 4.34 8.745-9.283 4.176 4.34 7.845-4.34-8.741 9.283Z"/></svg>
					<span class="oo-desktop-cta__tooltip">Messenger</span>
				</a>
			</div>';
...
function cnb_update_options() {
	...
	if(cnb_update_needed()) { // Check current version and if it needs an update
		...
		$default_options = array(
							  ...
							  'zalo' => isset($cnb_options['zalo']) ? $cnb_options['zalo'] : '',
							  'messenger' => isset($cnb_options['messenger']) ? $cnb_options['messenger'] : '',
